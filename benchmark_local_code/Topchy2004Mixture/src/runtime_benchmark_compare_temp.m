benchmark_first_iter = @(values) values(1);
addpath(genpath(pwd)); 
addpath('../utils/');
addpath('../metric/');
...
for dataIdx = benchmark_data_idx
    dataIdx
    ...
    for idx = 1:cntTimes
        ...
        [results1] = main(clusterings, M, k);
        benchmark_runtime_seconds = toc(benchmark_timer);
        save(benchmark_output_file, 'benchmark_runtime_seconds');
        error('BENCHMARK_RUNTIME:Completed', 'Benchmark runtime captured.');
    end
